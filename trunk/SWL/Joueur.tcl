...
# Partie graphique
 set this(root) [B_polygone]
   set this(poly_color)    [B_polygone]
     set this(rap_color) [B_rappel [Interp_TCL] "$objName set_color \[tk_chooseColor\]"]
...
#_________________________________________________________________________________________________________
Generate_accessors     Joueur [list nom color root_for_vaisseaux poly_vaisseau]
...
method Joueur set_color {args} {
 if {[llength $args] == 4} {
   ...
   set b [lindex $args 2]
   set a [lindex $args 3]
  } else {set r [expr Ox[string range $args 1 2] / 255.0]
          set v [expr Ox[string range $args 3 4] / 255.0]
		  set b [expr Ox[string range $args 5 6] / 255.0]
		  set a 1
         }
...
method Joueur Add_L_vaisseaux {LV} {
 ...
 foreach V $LV {
   $V set_mode $this(mode)
   $this(root_for_vaisseaux) Ajouter_fils_au_debut [$V get_root]
   [$V get_root] Ajouter_MetaData_T Joueur $objName
  }
}
#_________________________________________________________________________________________________________
method Joueur set_mode {m} {
 set this(mode) $m
 if {$m == "jeu"} {set v 0} else {set v 1}
 $this(root)    Gerer_contacts $v
 $this(txt_nom) Editable       $v
 foreach V $this(L_vaisseaux) {$V set_mode $m}
}
