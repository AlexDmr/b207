...
SDL_Surface *CreateLight(SDL_Surface *screen, int radius)
{
...
#endif
	pixel = SDL_MapRGB(light->format, 0xFF, 0xDD, 0x88);
	for ( y=0; y<light->h; ++y ) {
		for ( x=0; x<light->w; ++x ) {
			*buf++ = pixel;
		}
	...
	for ( y=0; y<light->h; ++y ) {
		for ( x=0; x<light->w; ++x ) {
			...
			/* Scale distance to range of alpha values (0-255) */
			if ( range > radius ) {
				alpha = alphamask;
			} else {
				/* Increasing transparency with distance */
				alpha = (Uint8)((range*alphamask)/radius);

				/* Lights are very transparent */
				addition = (alphamask+1)/8;
				if ( (int)alpha+addition > alphamask ) {
					alpha = alphamask;
				} else {
					alpha += addition;
				}
...
fprintf(stderr, "Slept %d ticks\n", (SDL_GetTicks()-ticks));
	...
	if ( flashes > 0 ) {
		printf("%d alpha blits, ~%d ms per blit\n", 
						flashes, flashtime/flashes);
	}
	exit(0);
}
