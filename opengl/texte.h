...
class gl_fonte
{
 private:
   ...
   HFONT hOldFont;
   int nb_char;
   float profondeur_fonte
       , hauteur;
   HDC hdc;
   GLYPHMETRICSFLOAT *g_GlyphInfo;

   UINT CreerFonteOpenGL(LPSTR NomFonte, float profondeur);
   float Calculer_hauteur();
...
*/
 public:
  ...
  // les méthodes.
   float ecrire(int &NbCarEcrits, const float taille_max, unsigned char *strString, ...);
   float longueur_effective(int &NbCarEcrits, const float taille_max, unsigned char *strText);
   ...
   inline const float PROFONDEUR() const {return profondeur_fonte;}
   inline const float HAUTEUR()    const {return hauteur;}
