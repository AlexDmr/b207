 ...
 set this(root)  [B_noeud]
   ...
   set this(space) [B_polygone]
	 ...
	 set contour [ProcRect 0 0 $X $Y]; 
	   ...
	   Drop_zones B [list [list $this(space) "$objName Is_a_dragged_class \$n" "$objName Add_new_instance" "$this(space) Couleur 0.2 0.4 0.2 1" "$this(space) Couleur 0 0 0 1"]
	                ]

	 Detruire $contour

 this set_mode edition
...
# Ici, on se contente d'ajouter un polygone de forme identique sur la zone de drop, à l'endroit précis du drop.
 ...
 switch $classe {
   Vaisseau {set joueur [$dragged_node Val_MetaData Joueur]
			 ...
			 $new_elements set_mode $this(mode_jeu)
            }
   Planete  {
            }
  }
...
#_________________________________________________________________________________________________________
#_________________________________________________________________________________________________________
method SWL set_mode_de_jeu {m} {
 set this(mode_jeu) $m
 $this(menu) set_mode $m
 foreach J $this(L_joueurs)  {$J set_mode $m}
 foreach P $this(L_planetes) {$P set_mode $m}
}
...
method SWL Add_L_joueurs {LJ} {
 ...
 foreach J $LJ {
   ...
   $J set_mode $this(mode_jeu)
   $this(ScrollList_Joueurs) Add_elements [$J get_root]
   puts "Vive le joueur $J _________________________________________"
   Drag_nodes [$J get_poly_vaisseau] 1 "puts coucou; $this(B_panel) toggle_extension" "$this(B_panel) toggle_extension"
  }
}
